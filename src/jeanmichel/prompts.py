...
def _render_output_contract_v2(role: str) -> str:
    ...
    if role == "specialist":
        return (
            ...
            "- If you are blocked specifically on a decision only the HUMAN can make (a choice, a "
            "preference, missing external information), first save any work-in-progress to the "
            "workspace, then conclude with `report_back(confidence='low', "
            "low_confidence_reason='HUMAN INPUT NEEDED: <the precise question>')`. The router relays it "
            "and re-delegates to you — you RESUME this exact task with the answer, so don't redo prior work.\n"
    ...
    if role == "router":
        return (
            ...
            "- When a specialist returns low with `HUMAN INPUT NEEDED: <Q>`, relay <Q> to the human via "
            "`ask_human` (question + choices in the user's language), then re-delegate to the SAME "
            "specialist — it resumes exactly where it left off with the answer (don't restate the whole task).\n"
