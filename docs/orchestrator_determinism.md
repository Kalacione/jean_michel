...
> Généré le 2026-06-12 19:13 UTC. Valeurs **lues en live** depuis `config` ; le registre des points de contrôle est ancré au code. Régénérer : `./jm.sh --orchestrator-map`.
>
...
| **Wall-clock** | | | |
| `LLM_CALL_TIMEOUT_SECONDS` | `120` | `JEANMICHEL_LLM_TIMEOUT` | single LLM call timeout |
| `REQUEST_WALL_CLOCK_SECONDS` | `900` | `JEANMICHEL_REQUEST_TIMEOUT` | one agent request budget |
| `TURN_WALL_CLOCK_SECONDS` | `1800` | `JEANMICHEL_TURN_TIMEOUT` | whole-turn budget (router + children) |
...
| `DEFAULT_MODEL_CONTEXT_WINDOW` | `128000` | `JEANMICHEL_DEFAULT_CTX_WINDOW` | assumed ctx window when a model is unknown |
| **Code mode** | | | |
...
| `REPO_PROTECTED_PATHS` | `jeanmichel.db, .env, .api_secret, conversations/, backups/, voice_models/, .venv/, graphify-out/, .git/` | `—` | paths repo_edit/repo_write must never touch |
| **Memory caps** | | | |
...
| `MAIN_MODEL` | `gemma4:latest` | `JEANMICHEL_MAIN_MODEL` | router default (non-code) |
| `CODE_MODEL` | `qwen3:14b` | `JEANMICHEL_CODE_MODEL` | router model in code mode |
| `SUBAGENT_DEFAULT_MODEL` | `gemma4:latest` | `JEANMICHEL_SUBAGENT_MODEL` | specialist default (unless model_override) |
| `COMPACTOR_MODEL` | `gemma4:latest` | `JEANMICHEL_COMPACTOR_MODEL` | LLM used for compaction levels 3-4 |
| **MCP** | | | |
...
Déclencheur de délibération : `complexity_probe` = keywords / >=2 files ; `MAX_REWORK` = 2.
...
| **Context packet (CRP)** | pipeline | `src/jeanmichel/context_packet.py` | — | deterministic per-delegation context (graphify + grep + source + git-diff + memory); code mode only. |
...
| **Deliberation trigger** | gate | `src/jeanmichel/deliberation.py · should_deliberate` | complexity_probe, MAX_REWORK | selective: code worker + worktree + hard step → thesis/antithesis/synthesis + KISS gate (bounded REWORK). |
