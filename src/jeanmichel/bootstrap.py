...
Mirror the static ``cli_profile.toml`` into the ``memory`` table (scope='user')
on the first run (cf. §11 ter A doc 06). Idempotent : if the user already holds
any memory, the function does nothing.

This is a transitional helper. Once the user has saved their own entries
via ``manage_memory``, ``cli_profile.toml`` becomes vestigial.
"""
...
def bootstrap_user_memory_from_profile(
    ...
    """Create a `user/personal-profile` memory entry for a user, from a profile.

    Scoped to ``user_id`` (``None`` → the reserved ``cli`` user). No-op when :
    - the ``memory`` table / ``cli`` user is missing (migrations not applied),
    - this user already holds at least one memory entry,
    ...
    try:
        ...
        _log.info(
            "bootstrap skipped : memory/web_users missing (migrations not applied)"
        )
        return False
    ...
    conn.execute(
        "INSERT INTO memory "
        "(scope, user_id, code, title, description, content, created_at, modified_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (_BOOTSTRAP_SCOPE, uid, _BOOTSTRAP_CODE, title, description, content, now, now),
    )
    return True
